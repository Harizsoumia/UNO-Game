...
// Classe abstraite Player (joueur générique)
abstract class Player {
    private String nom; // Nom du joueur
    private ArrayList<UnoCard> playerCards; // Liste des cartes du joueur
    private int nbrCards; // Nombre de cartes du joueur
    ...
    // Getter pour les cartes du joueur
    public ArrayList<UnoCard> getPlayerCards() {
        return playerCards;
    }

    // Ajoute une carte à la main du joueur
    public void ajouterCarte(UnoCard card) {
        playerCards.add(card);
        nbrCards++;
    }

    // Joue une carte de la main
    public UnoCard lancerCarte(int index) {
    ...
    public void afficherNombreDeCartes() {
        System.out.println("Nombre de cartes : " + nbrCards);
    }
    ...
    public void verifierGagnant() {
        if (nbrCards == 0) {
            ...
            System.out.println(nom + ", tu n'as pas encore gagné.");
        }
    }

    // Méthode abstraite pour jouer une carte (sera redéfinie par les sous-classes)
    public abstract UnoCard jouer();
}

// Classe Bot qui hérite de Player
class Bot extends Player {

    // Constructeur pour Bot
    ...
    @Override
    public UnoCard jouer() {
        if (getPlayerCards().size() > 0) {
            // Le bot joue toujours la première carte disponible
            return lancerCarte(0);
        } else {
            System.out.println("Le bot " + getNom() + " n'a plus de cartes à jouer.");
            return null;
        }
    }

    // Méthode supplémentaire spécifique au Bot
    public void analyserSituation() {
        System.out.println("Le bot " + getNom() + " analyse la situation...");
    }
}
